...
def initial(T_ref, section_0, section_1, MassFlow, input_power, input_pressure, A1, dh, \
    ...
    for i in range( len(section_1)-1 ) :
        ...
        Nu, re, pr, h_1, v_s = nusselt(MassFlow, T_ref[i+1], A1, \
        input_pressure, dh, epsi, deltaz)

        T_metal[i+1] = T_ref[i+1] + input_power[i]/(film_coeff(dh,\
        ...
        h_f.append(h_1)
        v_secc.append(v_s)
        htc_0.append(film_coeff(dh, input_pressure, T_ref[i+1], Nu) \
        * A2)
...
def looper(T_ref, section_0, section_1, MassFlow, input_power, Ag, dh, epsi, \
    ...
    while sqrt((1-(P_secc[len(P_secc)-1]/P_secc_old[len(P_secc_old)-1]))**2) > 10E-8:
        ...
        for i in range( len(section_1)-1 ):
            ...
            Nu, re, pr, h_1, v_s = nusselt(MassFlow, T_ref[i+1], Ag[i+1], \
            ...
            h_f.append(h_1)
            v_secc.append(v_s)
            htc_0.append(film_coeff(dh, P_secc[i], T_ref[i], Nu) \
