...
class Group:
    ...
    def __init__(self, input_data, cells):
        ...
        working_fluid.fluid_properties(self.group_data["input_pressure"], self.group_data["input_temperature"])
    ...
    def create_DLH_first_channel(cls, input_data, cells):
        ...
        if obj.group_data["distribution_type"] == "strikepoint":
            reordered_hexagons = sorted(cells.heat_cells[0], key=lambda x: x.R1, reverse=False)
            ...
            reordered_hexagons = sorted(cells.heat_cells[0], key=lambda x: x.R3, reverse=False)
            hexagon_selection = reordered_hexagons[:int(obj.group_data["number_of_hexagons_in_first_channel"])]
    ...
    def create_DLH_structured_channel(cls, input_data, cells):
        ...
        input_data["input_pressure"] = input_data["output_pressure"]
        input_data["input_temperature"] = input_data["output_temperature"]

        obj = cls(input_data, cells)

        if obj.group_data["specified_number_of_hexagons"] is None:
            obj.group_data["specified_number_of_hexagons"] = len(cells.heat_cells[obj.group_data["direction"]])

        if obj.group_data["specified_number_of_hexagons"] >= len(cells.heat_cells[obj.group_data["direction"]]):
            obj.group_data["specified_number_of_hexagons"] = len(cells.heat_cells[obj.group_data["direction"]])
        ...
        # hexagon selection incorrect
        obj.get_hexagons(obj.group_data, cells, hexagon_selection)

        obj.group_data["output_pressure"] = input_data["input_pressure"] - obj.group_data["pressure_drop"]
        obj.group_data["output_temperature"] = sum([hexagon.developed_coolant_temperature for hexagon in
                 obj.group_data["hexagons"]]) / obj.group_data["specified_number_of_hexagons"]
    ...
    @classmethod
    def create_DLH_HF_specific_channel(cls, input_data, cells):

        obj = cls(input_data, cells)

        mass_flow_next_row(obj.group_data, cells)
...
class CellManipulation:
    ...
    def _populate_cells(self, input_data): # this is essentially a manager function
        ...
        for i, r in enumerate(rset):
            if len(rset) - 1 == i:
                ...
                cp_final_y, assessment_set = self._bounding_box(input_data, r, rset[+1], \
    ...
    def _remove_external(self, cp, topy, first_radius, point_x1, point_y1, \
                         line_angle, input_data, second_radius):
        ...
        for coordinates in cp:
            ...
            elif arc(first_radius, coordinates[1]) >= coordinates[0]:
                yield coordinates

            elif arc(second_radius, coordinates[1]) <= coordinates[0]:
                yield coordinates
    ...
    def _bounding_box(self, input_data, first_radius, second_radius, cp_final_y, \
        ...
        cp = self._starter(input_data, cp, point_x1, topy)
        cp = self._remove_external(cp, topy, first_radius, point_x1, point_y1, \
                          line_angle, input_data, second_radius)
...
class DLHCells(CellManipulation):
    ...
    def _starter(self, input_data, cp, point_x1, topy):
        ...
        no_points_y = int(round(topy/height, 0))
        no_points_x = int(round(((input_data["outer_radius"]-point_x1) / (1.5*width)), 0))
    
        input_data["cross_sectional_area"] = (3 * sqrt(3) / 2) * (width/2) ** 2
    
        for i in range(no_points_x):
            for j in range(no_points_y):
                yield [i*1.5*width + point_x1, j*height]
                yield [i*1.5*width + 0.75*width + point_x1, j*height + 0.5*height]





# def starter(input_data):    
# width = input_data["width"] + 2 * input_data["thickness"]
#             height = width / 2 * tan(60 * pi / 180)
